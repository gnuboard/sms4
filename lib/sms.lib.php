...
class SMS4 {
	...
	function SMS_con($host,$id,$pw,$portcode) {
		if ($portcode == 1) { 
			$port=(int)rand(7192,7195); // 충전식
		} else {
			$port=(int)rand(7196,7199); // 정액제
		}
	...
	function Add($strDest, $strCallBack, $strCaller, $strURL, $strMessage, $strDate="", $nCount) {
		...
		for ($i=0; $i<$nCount; $i++) {
			...
			if (!$strURL) {
				...
				$this->Data[$i]	= '01144 '.$this->icode_id.$this->icode_pw.$hp_number.$strCallBack.$strCaller.$strDate.$strData;
			} else {
				$strURL		= FillSpace($strURL,50);
				$strData	= FillSpace(CheckCallCenter($strURL, $hp_number, $strData),80);

				$this->Data[$i]	= '05173 '.$this->icode_id.$this->icode_pw.$hp_number.$strCallBack.$strURL.$strDate.$strData;
			}
		}
		return true; // 수정대기
	}
...
function CheckCommonTypeDest($strDest, $nCount) {
	for ($i=0; $i<$nCount; $i++) {
		$hp_number = eregi_replace("[^0-9]","",$strDest[$i][bk_hp]);
